...
export default function ListPertanyaan({ level, isDelete, setDataDelete, setIsDelete }) {
    ...
    const fetchData = async () => {
        try {
            const response = await axiosInstance.get(`/admin/questions?level=${level}&page=${halaman}&limit=5`);
            ...
            {loading ? (
                ...
                <>{
                    data == 0 ? (
                        ...
                        <>
                            <div className="overflow-hidden rounded-lg shadow-lg mx-5 mt-7 min-h-1/2">
