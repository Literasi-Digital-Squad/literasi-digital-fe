...
export default function Question({ nomor_soal }) {
    ...
    const handleNext = async () => {
        if (!selectedJawaban || summary.length === maxQuestion) return
        setLoading(true)
                ...
                <div className="space-y-3">
                    {answers.map((answer) => (
                        <div
                            ...
                            className={`p-3 rounded-md w-full shadow-sm cursor-pointer border ${
                                selectedJawaban === answer.id ? "bg-[#0A2D61] text-white" : "bg-white"
                            }`}
                        >
                            {answer.image_url && (
                                <div className="mb-2">
